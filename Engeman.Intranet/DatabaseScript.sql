...
CREATE TABLE FORBIDDENWORD ( 
	...
	CONSTRAINT PK_FORBIDDENWORD PRIMARY KEY CLUSTERED(ID),
...
CREATE TRIGGER FORBIDDENWORD_CHANGEDATE ON FORBIDDENWORD AFTER UPDATE
AS 
 SET NOCOUNT ON
BEGIN
  UPDATE FORBIDDENWORD SET CHANGE_DATE = CURRENT_TIMESTAMP FROM FORBIDDENWORD
  INNER JOIN INSERTED ON FORBIDDENWORD.ID = INSERTED.ID;
END
GO
